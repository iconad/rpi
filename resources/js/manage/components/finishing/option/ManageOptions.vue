<template>
    <div>
        <div class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200 flex items-center justify-between">
            <span>options</span>
            <button @click="sideBar('create-option')" class="text-sm font-semibold block theme-link">Add New Option</button>
            </div>
            <!-- New Table -->
                        ...
                        <tr class="text-gray-700 dark:text-gray-400" v-for="(option, i) in options" :key="i">
                            ...
                            <td class="px-4 py-3 text-xs">
                                <span class="flex items-center">
                                    <button class="action-button-warning" @click="sideBarEdit('edit-option', option, i)">Edit</button>
                                    <delete-record @updated="updateTable" :id="finishid" :id2="option.id" model="finishings" model2="options"  class="ml-3"></delete-record>
                                </span>
                            </td>
                        </tr>
        ...
        <side-bar :finishid="finishid" @updated="updateTable"></side-bar>
    </div>
</template>
<script>
    ...
    import options from "../../../../../../gql/queries/finishingoptions.gql";
        ...
        apollo: {
            options() {
                return {
                    query: options,
                    variables: {
                        finish_id: this.finishid
                    },
                    update(data) {
                        return data.finishingoptions;
                    },
                };
            },
        }
