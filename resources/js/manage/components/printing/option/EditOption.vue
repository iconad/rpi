<template>
    <div>
        <div class="p-6 relative w-full">

            <div class="text-xl mb-3 capitalize font-semibold">Edit Option</div>
            <!-- <hr class="my-2"> -->
            <ValidationObserver v-slot="{ invalid,passes }">
            <form @submit.prevent="submitForm">
                <div class="form-element">
                    <label class="w-full block">
                        <span class="text-sm font-medium mb-1">Title</span>
                        <ValidationProvider name="form.title" rules="required">
                            <div slot-scope="{ errors }">
                                <input type="text" class="form-input text-lg" v-model="editingTable.data.title">
                                <p class="text-theme-red-light mt-1 px-1 text-sm font-medium">{{ errors[0] }}</p>
                            </div>
                        </ValidationProvider>
                    </label>
                </div>
                <div class="form-element">
                    <label class="w-full block">
                        <span class="text-sm font-medium mb-1">Price</span>
                        <input type="number" class="form-input text-lg" v-model="editingTable.data.price">
                    </label>
                </div>
                <div class="form-element">
                    <label class="w-full block">
                        <span class="text-sm font-medium mb-1">Extra Working Days</span>
                        <input type="number" min="0" class="form-input text-lg" v-model="editingTable.data.days">
                    </label>
                </div>
                <div class="form-element">
                    <label class="w-full block">
                        <span class="text-sm font-medium mb-1">Size</span>
                        <ValidationProvider name="editingTable.data.size" rules="required">
                            <div slot-scope="{ errors }">
                                <input type="text" class="form-input text-lg" placeholder="15x10cms" v-model="editingTable.data.size">
                                <p class="text-theme-red-light mt-1 px-1 text-sm font-medium">{{ errors[0] }}</p>
                            </div>
                        </ValidationProvider>
                    </label>
                </div>
                <div class="form-element">
                    <label class="w-full block relative">
                        <span class="text-sm font-medium mb-1">Type</span>
                        <ValidationProvider name="editingTable.data.type" rules="required">
                            <div class="relative">
                                <select class="form-input text-lg capitalize" v-model="editingTable.data.type">
                                    <!-- <template v-if="editingTable.data.type == type"> -->
                                        <option v-for="(type, i) in types" :key="i"> {{ type }} </option>
                                    <!-- </template> -->
                                </select>
                                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                </div>
                            </div>
                        </ValidationProvider>
                    </label>
                </div>
                <div class="form-element">
                    <label class="w-full block text-left">
                        <span class="text-sm font-medium mb-2 block">Selection Type </span>
                        <input type="checkbox" class="form-input w-5 h-5" v-model="editingTable.data.selection_type">
                    </label>
                </div>
                <div class="form-element">
                    <div class="font-medium text-sm">Current Thumbnail</div>
                    <div class="mb-3 w-24">
                        <thumb-image v-if="editingTable.data.media.length != 0" classess="w-full" :image="firstMedia[0].file_name" :id="firstMedia[0].id"></thumb-image>
                        <span class="text-xs italic" v-else> No Thumbnail  </span>
                    </div>
                    <label class="w-full block">
                        <span class="text-sm font-medium mb-1">Change Thumbnail</span>
                        <input type="file" @change="addImage" name="image" class="form-input text-lg">
                    </label>
                </div>
                <div v-if="loading">
                    <div class="loader"></div>
                </div>
                <div v-else class="mt-8">
                    <div class="w-32">
                        <button
                        type="submit"
                        :disabled="invalid"
                        :class="invalid ? 'bg-gray-200 text-gray-700 border-gray-200 hover:border-gray-200 hover:bg-gray-200 cursor-not-allowed' : '' "
                        class="theme-button text-base font-medium"
                        >Update</button>
                    </div>
                </div>

            </form>
            </ValidationObserver>
        </div>
    </div>
</template>

<script>

    import ThumbImage from '../../../../components/ThumbImage'

    import { extend,ValidationProvider,ValidationObserver } from 'vee-validate';
    import { required, oneOf } from 'vee-validate/dist/rules';

    extend('required', {
        ...required,
        message: 'This field is required'
    });


    export default {
        props: ['printid'],
        components: {
            ThumbImage,
            ValidationProvider,
            ValidationObserver,
        },
        data() {
            return {
                image: null,
                loading: false,
                types: ['front side', 'back side', 'sleeves']
            }
        },
        computed: {
            firstMedia (){
                return this.editingTable.data.media.filter(e => {
                    if(e.collection_name === 'images') {
                        return e
                    }
                })
            },
            editingTable: {
                get: function () {
                    return this.$store.state.editingTable
                },
                set: function (value) {
                    console.log(value)
                }
            },
        },
        methods: {
            addImage(e) {
                var fileReader = new FileReader()
                fileReader.readAsDataURL(e.target.files[0]);
                fileReader.onload = (e) => {
                    this.image = e.target.result;
                }
                this.image = e.target.files[0];
            },
            submitForm () {
                this.loading = true
                axios.put(`/manage/printing/${this.printid}/options/${this.editingTable.data.id}`, {
                    title: this.editingTable.data.title,
                    price: this.editingTable.data.price,
                    type: this.editingTable.data.type,
                    selection_type: this.editingTable.data.selection_type,
                    size: this.editingTable.data.size,
                    days: this.editingTable.data.days,
                    image: this.image,
                })
                .then(response => {
                    this.$emit('updated')
                    this.$swal({
                        toast: true,
                        position: 'center',
                        showConfirmButton: false,
                        timer: 3000,
                        title: 'Printing',
                        text: 'Updated!',
                    });
                    this.loading = false
                })
            },
        },

    }
</script>

<style lang="scss">
.pretty .state label:before, .pretty .state label:after {
    top: 0px!important;
}
</style>
