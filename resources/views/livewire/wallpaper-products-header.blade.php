        ...
        <div class="search w-20rem flex items-center mt-5 lg:mt-0 lg:pb-5">
            ...
            <div class="w-full">
                ...
                @if(isset($_GET['materials']))
                    @foreach ($_GET['materials'] as $material)
                        <input class="hidden" type="checkbox" name="materials[]" value="{{$material}}" checked>
                    @endforeach
                @endif
                @if(isset($_GET['sort']))
                ...
                <input type="search" value="@if(isset($_GET['search'])){{$_GET['search']}}@endif" placeholder="Search for {{$subcategory->title}} " class="w-full border-b border-gray-500  bg-transparent focus:outline-none" name="search" id="search">
                </form>
            </div>
        </div>
    </div>
