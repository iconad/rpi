        ...
        <form action="{{route('categories.store')}}" method="post" enctype="multipart/form-data" class="w-full rounded-lg border border-gray-200 bg-white p-4">
            ...
            <div class="form-element">
                <label class="block mb-3">
                    ...
                    <span>Select Cover from Product</span>
                </label>
                <label class="block mb-3">
            ...
            <div class="form-element">
                ...
                <div class="relative">
                    <select wire:model="selectedProduct" wire:change="onSelectProduct" class="form-input text-base capitalize w-64" name="menu_id">
                        @foreach ($products as $product)
                            <option value="{{$product}}" class="capitalize"> {{$product->title}} </option>
                        @endforeach
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                    </div>
                </div>
                {{-- {{is_string($selectedProduct)}} --}}
                {{$cover}}


                {{-- <thumb-image-blade classess="w-1/4" image="{{$selectedProduct->media[0]->file_name}}" id="{{$selectedProduct->media[0]->id}}"></thumb-image-blade> --}}
