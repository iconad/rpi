...
<section class="pages-wrapper my-10 flex flex-col space-y-6">
    ...
    <div class="categories">
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-5">
            @foreach ($categories as $catg)
                <a href="/layout-templates/{{$catg->slug}}" class="category-box border hover:bg-orange-100 transition bg-gray-100 {{$category->slug === $catg->slug ? 'bg-orange-100' : ''}} ">
                    <span class="image h-32 md:h-48 p-5 flex items-center justify-center">
                        <thumb-image-blade classess="w-4/5 md:w-full h-full object-cover" image="{{$catg->media[0]->file_name}}" id="{{$catg->media[0]->id}}"></thumb-image-blade>
                    </span>
    ...
    <div>
        ...
        <div class="templates border border-primary-300 mt-3 rounded border-b-0">
            <div class="bg-primary-500 text-center text-gray-100 font-medium text-lg flex items-center">
            ...
            @foreach ($templates as $template)
                <div class="flex items-center h-auto border-b border-primary-300">
                    <div class="w-22rem px-6 py-3 flex items-center justify-center border-r border-primary-300">
                        <thumb-image-blade classess="w-5/6 object-cover" image="{{$template->media[0]->file_name}}" id="{{$template->media[0]->id}}"></thumb-image-blade>
                    </div>
