    ...
    <div class="w-full rounded-lg border border-gray-200 bg-white p-4 relative">
        <form action="{{route('products.update', $product->id)}}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-element mt-5">
                <label for="subcateogry" class="flex items-center space-x-2">
                    @if ($product->rule_of_120 == 'on')
                    <input type="checkbox" name="rule_of_120" checked class="h-4 w-4">
                    @else
                    <input type="checkbox" name="rule_of_120" class="h-4 w-4">
                    @endif
                    <span class="text-gray-800 inline-block pt-1">The Rule of 120</span>
                </label>
            </div>
            <!-- form-ele -->
